...
class UpdateSubject extends Component {
    ...
    componentDidMount() {
        ...
        const path = 'http://localhost:8080/subjects/'+this.props.match.params.id;
    ...
    handleSubmit = (event) => {
        const requestOptions = {
            ...
            body: JSON.stringify({
                code: this.state.subject.code,
                name: this.state.subject.name
            })
        };
        
        const path = 'http://localhost:8080/subjects/' + this.state.subject.id;
        fetch( path, requestOptions)
        .then(response => 
            {
                if(response.ok) {
